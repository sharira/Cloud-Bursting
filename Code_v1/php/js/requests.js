...
function resetAWS(){
	console.log('resetAWS');
	AWS=0;
}

function mySuggestedTimes(){
//function mySuggestedTimes(unityiddata) {
	console.log('mysuggestedtime');
	AWS = 1;
	//console.log(unityiddata);
	//unityid = unityiddata;
...
function submitNewReservation() {
	...
	if(AWS == 1)
	{
		console.log("AWS condition");
	//	console.log(unityid);
		console.log(dojo.byId('deployerr').innerHTML);
		console.log(data.duration);
		//exec('ls');

/*		var exec = require('child_process').exec, child;

		child = exec('python /js/dumm.py',
...
/*		var xhr = new XMLHttpRequest();
		xhr.open("POST", "https://152.46.17.18:8080/vcl/js/dummy_server.py", true);
        	url: "../../cgi-bin/hello.py?username=&duration=data.duration",
*/			
		var myuser = dojo.byId('deployerr').innerHTML;
		var myduration = data.duration;
		var myurl="../../cgi-bin/hello.py?username="+myuser+"&duration="+data.duration;
		var myurl2="../../cgi-bin/hello2.py";
		$.ajax({
		url: myurl,
        	type: "POST",
